...
@st.cache
def load_data():
    df_client = pd.read_csv('P7_OpenClassrooms/api/static/infos_clients.csv')
    df_client.drop(columns=['Unnamed: 0'], inplace=True)

    df_pret = pd.read_csv('P7_OpenClassrooms/api/static/infos_prets.csv')
    df_pret.drop(columns=['Unnamed: 0'], inplace=True)

    df_predict = pd.read_csv('P7_OpenClassrooms/api/static/data_preprocessed.csv')
